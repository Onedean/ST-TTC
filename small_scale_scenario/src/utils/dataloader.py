...
def load_dataset(data_path, args, logger):
    folder_name = args.years+f'-fewshot{args.fewshot}' if args.fewshot!=100 else args.years
    
    if args.seq_len != 12:
        folder_name = folder_name + '-long'
        
    ptr = np.load(os.path.join(data_path, folder_name, 'his.npz'), allow_pickle=True)
    logger.info('Data shape: ' + str(ptr['data'].shape))
    dataloader = {}
    for cat in ['train', 'val', 'test']:
        idx = np.load(os.path.join(data_path, folder_name, 'idx_' + cat + '.npy'))
        dataloader[cat + '_loader'] = DataLoader(ptr['data'][..., :args.input_dim], idx, args.seq_len, args.horizon, args.bs, logger)
...
def get_dataset_info(dataset):
    ...
    d = {
        ...
        'KNOWAIR': [base_dir+'knowair', base_dir+'knowair/knowair_rn_adj.npy', 184],
        'URBANEV': [base_dir+'urbanev', base_dir+'urbanev/urbanev_rn_adj.npy', 275],
        }
