...
class STAEformer(BaseModel):
    def __init__(self, steps_per_day, input_embedding_dim, tod_embedding_dim, dow_embedding_dim, spatial_embedding_dim, adaptive_embedding_dim, feed_forward_dim, num_heads, num_layers, dropout, use_mixed_proj, **args):
        ...
        if spatial_embedding_dim > 0:
            self.node_emb = nn.Parameter(
                torch.empty(self.num_node, self.spatial_embedding_dim)
            )
            nn.init.xavier_uniform_(self.node_emb)
        if adaptive_embedding_dim > 0:
            self.adaptive_embedding = nn.init.xavier_uniform_(
                nn.Parameter(torch.empty(self.seq_len, self.node_num, adaptive_embedding_dim))
            )

        if use_mixed_proj:
            self.output_proj = nn.Linear(
                self.seq_len * self.model_dim, self.horizon * self.output_dim
            )
        else:
            self.temporal_proj = nn.Linear(self.seq_len, self.horizon)
    ...
    def forward(self, x, label=None):
        ...
        if self.spatial_embedding_dim > 0:
            spatial_emb = self.node_emb.expand(
                batch_size, self.seq_len, *self.node_emb.shape
            )
        ...
        if self.use_mixed_proj:
            ...
            out = out.reshape(
                batch_size, self.node_num, self.seq_len * self.model_dim
            )
            out = self.output_proj(out).view(
                batch_size, self.node_num, self.horizon, self.output_dim
            )
