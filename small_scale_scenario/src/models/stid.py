...
class STID(BaseModel):
    ...
    def __init__(self, node_dim, input_len, input_dim, embed_dim, output_len, num_layer, temp_dim_tid, temp_dim_diw, time_of_day_size, day_of_week_size, if_T_i_D, if_D_i_W, if_node, **args):
        ...
        # self.time_series_emb_layer = nn.Conv2d(in_channels=1 * self.input_len, out_channels=self.embed_dim, kernel_size=(1, 1), bias=True)
    ...
    def forward(self, input, label=None):  # (b, t, n, f)
        ...
        input_data = input_data.transpose(1, 2).contiguous()  # [b, t, n, f] -> [b, n, t, f]
        input_data = input_data.view(batch_size, num_nodes, -1).transpose(1, 2).unsqueeze(-1)  # [b, n, t, f] -> [b, n, t * f] -> [b, t * f, n] -> [b, t * f, n, 1]
        time_series_emb = self.time_series_emb_layer(input_data)  # [b, t * f, n, 1] -> [b, emb_dim, n, 1]
        
        node_emb = []
        if self.if_spatial:
            # expand node embeddings
            node_emb.append(self.node_emb.unsqueeze(0).expand(batch_size, -1, -1).transpose(1, 2).unsqueeze(-1))  # [n, node_dim] -> [1, n, node_dim] -> [b, n, node_dim] -> [b, node_dim, n, 1]
